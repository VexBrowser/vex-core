...
constexpr char kRemainingCredentialCountKey[] = "remaining_credential_count";
}  // namespace
...
std::optional<CredentialSummary> CredentialSummary::FromMessage(
  ...
  summary.active_ = records->FindBool(kActiveKey).value_or(false);
  summary.remaining_credential_count_ =
      records->FindInt(kRemainingCredentialCountKey).value_or(0);
  return summary;
}
