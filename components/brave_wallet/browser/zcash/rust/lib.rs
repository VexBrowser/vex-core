...
mod ffi {
    ...
    extern "Rust" {
        ...
        ) -> Box<CxxOrchardExtendedSpendingKey>;
        ...
        ) -> Box<CxxOrchardUnauthorizedBundle>;
        ...
        fn complete(
            ...
            sighash: [u8; 32],
        ) -> Box<CxxOrchardAuthorizedBundleResult>; // Array size should match kZCashDigestSize
        ...
        ) -> Box<CxxOrchardShardTreeCapResultWrapper>;
        ...
        ) -> Box<CxxCheckpointBundleResultWrapper>;
        ...
        fn wrap_checkpoints(
            item: Vec<CxxOrchardCheckpointBundle>,
        ) -> Box<CxxCheckpointsResultWrapper>;
    ...
    unsafe extern "C++" {
        ...
        fn LastShard(&self, shard_level: u8) -> Box<CxxOrchardShardResultWrapper>;
        fn GetShard(&self, addr: &CxxOrchardShardAddress) -> Box<CxxOrchardShardResultWrapper>;
        fn PutShard(&self, tree: &CxxOrchardShard) -> Box<CxxBoolResultWrapper>;
        fn GetShardRoots(&self, shard_level: u8) -> Box<CxxShardRootsResultWrapper>;
        fn Truncate(&self, address: &CxxOrchardShardAddress) -> Box<CxxBoolResultWrapper>;
        fn GetCap(&self) -> Box<CxxOrchardShardTreeCapResultWrapper>;
        fn PutCap(&self, tree: &CxxOrchardShardTreeCap) -> Box<CxxBoolResultWrapper>;
        fn MinCheckpointId(&self) -> Box<CxxCheckpointIdResultWrapper>;
        fn MaxCheckpointId(&self) -> Box<CxxCheckpointIdResultWrapper>;
        fn AddCheckpoint(
            &self,
            checkpoint_id: u32,
            checkpoint: &CxxOrchardCheckpoint,
        ) -> Box<CxxBoolResultWrapper>;
        fn UpdateCheckpoint(
            &self,
            checkpoint_id: u32,
            checkpoint: &CxxOrchardCheckpoint,
        ) -> Box<CxxBoolResultWrapper>;
        fn CheckpointCount(&self) -> Box<CxxCheckpointCountResultWrapper>;
        fn CheckpointAtDepth(&self, depth: usize) -> Box<CxxCheckpointBundleResultWrapper>;
        fn GetCheckpoint(&self, checkpoint_id: u32) -> Box<CxxCheckpointBundleResultWrapper>;
...
impl_result!(
    CxxOrchardExtendedSpendingKey,
    CxxOrchardExtendedSpendingKeyResult,
    OrchardExtendedSpendingKey
);
impl_result!(
    CxxOrchardAuthorizedBundle,
    CxxOrchardAuthorizedBundleResult,
    OrchardAuthorizedBundleValue
);
impl_result!(
    CxxOrchardUnauthorizedBundle,
    CxxOrchardUnauthorizedBundleResult,
    OrchardUnauthorizedBundleValue
);
impl_result!(
    CxxOrchardDecodedBlocksBundle,
    CxxOrchardDecodedBlocksBundleResult,
    OrchardDecodedBlocksBundleValue
);
impl_result!(CxxOrchardShardTree, CxxOrchardShardTreeResult, OrchardShardTreeValue);
impl_result!(
    CxxOrchardTestingShardTree,
    CxxOrchardTestingShardTreeResult,
    OrchardTestingShardTreeValue
);
...
impl_result_wrapper!(bool, CxxBoolResultWrapper, wrap_bool, wrap_bool_error);
impl_result_wrapper!(
    usize,
    CxxCheckpointCountResultWrapper,
    wrap_checkpoint_count,
    wrap_checkpoint_count_error
);
impl_result_wrapper!(
    Vec<CxxOrchardCheckpointBundle>,
    CxxCheckpointsResultWrapper,
    wrap_checkpoints,
    wrap_checkpoints_error
);
impl_result_wrapper!(
    Vec<CxxOrchardShardAddress>,
...
impl CxxOrchardExtendedSpendingKey {
    fn derive(
        ...
        Box::new(CxxOrchardExtendedSpendingKeyResult::from(
            self.0.derive_child(OrchardChildIndex::hardened(index)).map_err(Error::from),
        ))
    }

    fn external_address(self: &CxxOrchardExtendedSpendingKey, diversifier_index: u32) -> [u8; 43] {
        let address =
            OrchardFVK::from(&self.0).address_at(diversifier_index, OrchardScope::External);
        address.to_raw_address_bytes()
    }

    fn internal_address(self: &CxxOrchardExtendedSpendingKey, diversifier_index: u32) -> [u8; 43] {
        let address =
            OrchardFVK::from(&self.0).address_at(diversifier_index, OrchardScope::Internal);
        address.to_raw_address_bytes()
    }

    fn full_view_key(self: &CxxOrchardExtendedSpendingKey) -> [u8; 96] {
        OrchardFVK::from(&self.0).to_bytes()
    }

    fn spending_key(self: &CxxOrchardExtendedSpendingKey) -> [u8; 32] {
        *self.0.sk().to_bytes()
    }
}
...
fn create_orchard_builder_internal(
    ...
    let anchor = if orchard_tree_bytes.len() > 0 {
        ...
        >(&orchard_tree_bytes[..])
        {
            ...
            Err(_e) => {
                return Box::new(CxxOrchardUnauthorizedBundleResult::from(Err(Error::from(
                    OrchardBuildError::AnchorMismatch,
                ))))
            }
        }
    } else {
    ...
    for spend in spends {
        ...
        let auth_path_sized: Result<[MerkleHashOrchard; orchard::NOTE_COMMITMENT_TREE_DEPTH], _> =
            auth_path.unwrap().try_into();
        if auth_path_sized.is_err() {
            return Box::new(CxxOrchardUnauthorizedBundleResult::from(Err(Error::WitnessError)));
        }

        let merkle_path =
            MerklePath::from_parts(spend.merkle_path.position, auth_path_sized.unwrap());

        let rho = Rho::from_bytes(&spend.rho);
        if rho.is_none().into() {
            return Box::new(CxxOrchardUnauthorizedBundleResult::from(Err(
                Error::OrchardActionFormatError,
            )));
        }
    ...
    Box::new(CxxOrchardUnauthorizedBundleResult::from(match random_source {
        OrchardRandomSource::OsRng(mut rng) => {
            builder.build(&mut rng).map_err(Error::from).and_then(|builder| {
                builder
                    .map(|bundle| OrchardUnauthorizedBundleValue {
...
fn batch_decode(
    ...
    {
        // If the commitment is the last in the block, ensure that is is retained as a
        // checkpoint
        ...
        if commitment.is_none().into() {
            return Box::new(CxxOrchardDecodedBlocksBundleResult::from(Err(
                Error::OrchardActionFormatError,
            )));
        }
...
impl CxxOrchardDecodedBlocksBundle {
    ...
    fn note_nullifier(self: &CxxOrchardDecodedBlocksBundle, index: usize) -> [u8; 32] {
        self.0.outputs[index]
            .note
            .nullifier(&OrchardFVK::from_bytes(&self.0.fvk).unwrap())
            .to_bytes()
    }
...
fn insert_frontier<const COMMITMENT_TREE_DEPTH: u8, const SHARD_HEIGHT: u8>(
    tree: &mut ShardTree<
        ShardStoreImpl<orchard::tree::MerkleHashOrchard, SHARD_HEIGHT>,
        COMMITMENT_TREE_DEPTH,
        SHARD_HEIGHT,
    >,
    frontier: &Vec<u8>,
) -> bool {
    let frontier_commitment_tree =
        read_commitment_tree::<MerkleHashOrchard, _, COMMITMENT_TREE_DEPTH>(&frontier[..]);
    ...
    let frontier_result =
        tree.insert_frontier(frontier_commitment_tree.unwrap().to_frontier(), Retention::Marked);

    frontier_result.is_ok()
}

fn insert_commitments<
    const COMMITMENT_TREE_DEPTH: u8,
    const CHUNK_SIZE: usize,
    const SHARD_HEIGHT: u8,
>(
    shard_tree: &mut ShardTree<
        ShardStoreImpl<orchard::tree::MerkleHashOrchard, SHARD_HEIGHT>,
        COMMITMENT_TREE_DEPTH,
        SHARD_HEIGHT,
    >,
    ...
    if !scan_result.0.prior_tree_state.frontier.is_empty() {
        let frontier_result = insert_frontier::<COMMITMENT_TREE_DEPTH, SHARD_HEIGHT>(
            shard_tree,
            &scan_result.0.prior_tree_state.frontier,
        );
        if !frontier_result {
            return false;
        }
...
impl CxxOrchardTestingShardTree {
    fn insert_commitments(
        ...
        insert_commitments::<TESTING_TREE_HEIGHT, TESTING_CHUNK_SIZE, TESTING_SHARD_HEIGHT>(
            &mut self.0.tree,
            scan_result,
        )
    }
...
impl From<&CxxOrchardCheckpoint> for Checkpoint {
    fn from(item: &CxxOrchardCheckpoint) -> Self {
        let tree_state: TreeState = if item.empty {
            TreeState::Empty
        } else {
            TreeState::AtPosition((item.position as u64).into())
        };
    ...
    fn try_from(item: &Checkpoint) -> Result<Self, Self::Error> {
        let position: u32 = match item.tree_state() {
            ...
            TreeState::AtPosition(pos) => {
                (u64::from(pos)).try_into().map_err(|_| Error::ShardStoreError)?
            }
        };
        let marks_removed: Result<Vec<u32>, Error> = item
            .marks_removed()
            .into_iter()
            .map(|x| u32::try_from(u64::from(*x)).map_err(|_| Error::ShardStoreError))
            .collect();
...
impl TryFrom<&Address> for CxxOrchardShardAddress {
    ...
    fn try_from(item: &Address) -> Result<Self, Self::Error> {
        ...
        Ok(CxxOrchardShardAddress { level: item.level().into(), index: index })
    }
}
...
impl<H: HashSer> TryFrom<&PrunableTree<H>> for CxxOrchardShardTreeCap {
    ...
    fn try_from(item: &PrunableTree<H>) -> Result<Self, Self::Error> {
        ...
        Ok(CxxOrchardShardTreeCap { data: data })
    }
}
...
impl<H: HashSer> TryFrom<&LocatedPrunableTree<H>> for CxxOrchardShard {
    ...
    fn try_from(item: &LocatedPrunableTree<H>) -> Result<Self, Self::Error> {
        ...
        let mut result = CxxOrchardShard {
            address: CxxOrchardShardAddress::try_from(&item.root_addr())
                .map_err(|_| Error::ShardStoreError)?,
...
impl<H: HashSer, const SHARD_HEIGHT: u8> ShardStore for ShardStoreImpl<H, SHARD_HEIGHT> {
    type H = H;
    ...
    fn put_shard(&mut self, subtree: LocatedPrunableTree<Self::H>) -> Result<(), Self::Error> {
        ...
        let result = *self.delegate.PutShard(&shard);
        if result.0.is_err() {
            return Err(Error::ShardStoreError);
        }
        Ok(())
    ...
    fn get_shard_roots(&self) -> Result<Vec<Address>, Self::Error> {
        ...
        return Ok(result
            ...
            .unwrap()
            .into_iter()
            .map(|addr| Address::from_parts(addr.level.into(), addr.index.into()))
            .collect());
    }
    ...
    fn add_checkpoint(
        ...
        let result = *self
            .delegate
            .AddCheckpoint(ffi_checkpoint_id, &CxxOrchardCheckpoint::try_from(&checkpoint)?);
        if result.0.is_err() {
            return Err(Error::ShardStoreError);
        }
        Ok(())
    ...
    fn get_checkpoint_at_depth(
        ...
        let result = *self.delegate.CheckpointAtDepth(checkpoint_depth);
        ...
        match result.0.unwrap() {
            Some(checkpoint_bundle) => {
                return Ok(Some((
                    BlockHeight::from(checkpoint_bundle.checkpoint_id),
                    Checkpoint::from(&checkpoint_bundle.checkpoint),
                )));
            }
            None => {
                return Ok(Option::None);
    ...
    fn get_checkpoint(
        ...
        let result = *self.delegate.GetCheckpoint((*checkpoint_id).into());
    ...
    {
        let result_get_checkpoint = self.delegate.GetCheckpoint((*checkpoint_id).into());
        ...
        let mut checkpoint =
            Checkpoint::from(&result_get_checkpoint.0.unwrap().unwrap().checkpoint);

        update(&mut checkpoint).map_err(|_| Error::ShardStoreError)?;
        let result_update_checkpoint = *self.delegate.UpdateCheckpoint(
            (*checkpoint_id).into(),
...
fn create_orchard_shard_tree(
    ...
    let shard_store = OrchardShardStoreImpl { delegate: delegate, _hash_type: Default::default() };
...
fn convert_ffi_commitments(
    shard_tree_leafs: &CxxOrchardShardTreeLeafs,
...
fn create_orchard_testing_shard_tree(
    ...
    let shard_store: ShardStoreImpl<MerkleHashOrchard, 4> =
        TestingShardStoreImpl { delegate: delegate, _hash_type: Default::default() };
    let shardtree = ShardTree::new(shard_store, TESTING_PRUNING_DEPTH.try_into().unwrap());
    Box::new(CxxOrchardTestingShardTreeResult::from(Ok(OrchardTestingShardTreeValue {
        tree: shardtree,
    })))
}

fn create_mock_commitment(position: u32, seed: u32) -> [u8; 32] {
    MerkleHashOrchard::from_bytes(&(pallas::Base::random(MockRng((position * seed).into())).into()))
        .unwrap()
        .to_bytes()
}
