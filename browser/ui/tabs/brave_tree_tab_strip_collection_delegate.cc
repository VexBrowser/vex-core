...
BraveTreeTabStripCollectionDelegate::CompactMovingTabs(
  ...
  for (auto* tab : moving_tabs) {
    ...
    if (compact_tree &&
        IsTreeNodeCoveredByMovingAncestor(
            GetParentTreeNodeCollectionOfTab(tab), moving_tabs_set)) {
      continue;
    }
...
void BraveTreeTabStripCollectionDelegate::MoveTabsRecursive(
  ...
  // of being flattened.
  const base::flat_set<tabs::TabInterface*> moving_tabs_set(
      moving_tabs.begin(), moving_tabs.end());
